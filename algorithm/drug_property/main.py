...
def drug_property_prediction(drug_smiles, task_list, batch_size, device):
    ...
    for task in task_list:
        ...
        best_model.load_state_dict(torch.load(model_file_path, map_location=torch.device(device)), strict=True)
...
if __name__ == "__main__":
    # 测试示例
    device = 'cuda:0'
    drug_smiles = ['CCO', 'invalid_smiles', 'C1=CC=CC=C1C']
    # task_list = ['BACE', 'ClinTox', 'FreeSolv']
    task_list = ['BACE', 'BBBP', 'ClinTox', 'SIDER', 'Tox21', 'ToxCast', 'ESOL', 'FreeSolv', 'LIPO']
    batch_size = 32
    properties = drug_property_prediction(drug_smiles, task_list, batch_size, device)
    print(properties)
