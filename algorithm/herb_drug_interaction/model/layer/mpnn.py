...
class KGMPNN(nn.Module):
    def __init__(self, model_args, entity_emb, relation_emb):
        super(KGMPNN, self).__init__()

        self.project_node_feats = nn.Sequential(
            nn.Linear(model_args['node_indim'], model_args['node_hidden_feats']),
            nn.ReLU()
        )
        self.num_step_message_passing = model_args['num_step_message_passing']
        attn_fc = nn.Linear(2 * model_args['node_hidden_feats'], 1, bias=False)
        edge_network1 = nn.Sequential(
            nn.Linear(model_args['edge_indim'], model_args['edge_hidden_feats']),
            nn.ReLU(),
            nn.Linear(model_args['edge_hidden_feats'], model_args['node_hidden_feats'] * model_args['node_hidden_feats'])
        )
        ...
        self.gnn_layer = KGMPNNLayer(
            in_feats=model_args['node_hidden_feats'],
            out_feats=model_args['node_hidden_feats'],
            attn_fc=attn_fc,
        ...
        self.gru = nn.GRU(model_args['node_hidden_feats'], model_args['node_hidden_feats'])
        self.out_dim = model_args['node_hidden_feats']

        atom_emb = torch.randn((118, model_args['node_indim']))
        node_emb = torch.cat((atom_emb, entity_emb), 0)
        bond_emb = torch.randn((4, model_args['edge_indim']))
