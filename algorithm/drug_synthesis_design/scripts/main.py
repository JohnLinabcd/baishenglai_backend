...
def Retrosynthetic_reaction_pathway_prediction(smiles_list, dataset, top_k_result, device, batch_size):
    args = {}
    args['mode'] = 'test'
    args['model'] = 'default'
...
if __name__ == '__main__':
    smile_list = ['C(OC(C(F)(F)F)C(F)(F)F)F', 'C1=NC(=NN1C2C(C(C(O2)CO)O)O)C(=O)N', 'invalid_smile', 'CC(C)NCC(COC1=CC=CC2=CC=CC=C21)O']
    dataset = 'USPTO_50K'  # or USPTO_MIT
    top_k_result = 3
    device = 'cuda:0'
    batch_size = 16
    result = Retrosynthetic_reaction_pathway_prediction(smile_list, dataset, top_k_result, device, batch_size)
    print(result)
    '''
    result = 
